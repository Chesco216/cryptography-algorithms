...
def _example_triple_des_():
	...
	# El ejemplo muestra el cifrado triple des utilizando la clase des.
	print ("Ejemplo de cifrado triple DES en modo ECB predeterminado (DES-EDE3)\n")

	print ("Triple des usando la clase des (3 veces)")
	t = time()
	...
	d2 = k2.encrypt(d3)
	d1 = k1.decrypt(d2)
	print ("Descifrada: %r" % d1)
	print ("DES time taken: %f (%d crypt operations)" % (time() - t, 6 * (len(d) / 8)))
	print ("")

	# El siguiente ejemplo utiliza la clase triple-des para lograr lo mismo que el anterior
	print ("Ahora usando triple des class")
	t = time()
	...
	td1 = t1.encrypt(d)
	print ("Encriptada: %r" % td1)

	td2 = t1.decrypt(td1)
	print ("Desencriptada: %r" % td2)
...
def _fulltest_():
	# Esto no debería producir errores o excepciones inesperados.
	from time import time
	from binascii import unhexlify as unhex
	...
	# Pruebe PAD_PKCS5 con el modo de cifrado CBC.
    ...
    #Asegúrese de que no se produzca ningún error al crear una instancia sin un IV aún establecido,
